<template>
	<div class="menu">
		<el-card class="box-card">
			...
			<!-- s表格 -->
			<el-table :default-expand-all="true" row-key="id" :data="tableData.data" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" style="width: 100%" size="small" highlight-current-row>

				<el-table-column prop="menu_name" label="菜单名称" min-width="100">
					<template v-slot="scope">
						<!--            <span>{{ scope.row.menu_name + '  [ ' + scope.row.id + '  ]' }}</span>-->
				...
				<el-table-column label="菜单归属" min-width="80" align="center">
					<template v-slot="scope">
						<!--            <div v-if="scope.row.pid === 0">-->
						<!--            <div :style="{paddingLeft:scope.row.pid !== 0 ? '30px' : '0'}">-->
						<div>
				...
				<el-table-column label="是否PC端" min-width="80" align="center">
					<template v-slot="scope">
						<!--            <div v-if="scope.row.pid === 0">-->
						<!--            <div :style="{paddingLeft:scope.row.pid !== 0 ? '30px' : '0'}">-->
						<div>
				...
				<el-table-column prop="is_show" label="是否启用" min-width="80" align="center">
					<template v-slot="scope">
						<div>
							<el-tag v-if="scope.row.is_show === 0" type="danger">否</el-tag>
				...
				<el-table-column prop="remark" label="备注" min-width="100">
					<template v-slot="scope">
						<el-tooltip class="item" effect="light" :content="scope.row.remark||'无'" placement="top">
		...
		<el-dialog title="添加菜单" :visible.sync="add_visible" :close-on-click-modal="false" width="50%">
			...
			<el-form ref="addForm" :model="addForm" :rules="addRules">
				...
				<el-form-item label="是否启用" :label-width="formLabelWidth" prop="is_show">
					<el-switch
	...
	data() {
		return {
			...
			tableData: {
				data: [ {
					icon: 'eleme',
					menu_name: '示例父级菜单（假数据）',
					remark: '',
					id: 11,
					children: [ {
						icon: 'eleme',
						menu_name: '示例子级菜单（假数据）',
						remark: '',
						id: 12
					} ]
				} ],
