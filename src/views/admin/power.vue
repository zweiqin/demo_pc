...
// 这里用的是GetHighestRole，而不是GetMenuByType。因为这个端，管理员有的，员工不一定有，这里显示的只是这个端的管理员所拥有的的路由，而不是这个端的全部路由。
//  另外，不用/v1/SysMenu/GetMenuList，是因为这个接口返回的数据没有做分类，即没有分成PcMenu和AppMenu
import { GetHighestRole, GetMenuByType } from '@/api/system_menu'
...
export default {
	...
	methods: {
		...
		async getMenuList(num, e) {
			await GetHighestRole({ role_type: num })
				.then((res) => {
					if (num === 0) {
						// 总后台
		...
		// 获取所有权限
		getRoleList() {
			GetRoleListNoRoot({ role_type: type, mer_id: localstorage.get('admin_info').mer_id, provider_id: localstorage.get('admin_info').provider_id, is_developers: 0 })
				.then((res) => {
					res.data.map((item) => {
		...
		async edit(e, meaning) {
			...
			// }, 2000)
			// this.$nextTick(() => {
			// 	this.pc_check = e.rules.split(',').map((item) => Number(item))
			// })
			this.$nextTick(() => {
